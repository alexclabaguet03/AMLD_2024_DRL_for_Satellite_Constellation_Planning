...
from amld2024.networks.policies import MLPPolicyPG
from amld2024.networks.critics import ValueCritic
...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounted_sum = rewards_array * (self.gamma ** np.arange(len(rewards)))
        discounted_return = discounted_sum.sum() * np.ones_like(rewards_array)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        # TODO: implement this function
        rewards_array = np.array(rewards)
        discounted_reward_to_go = np.zeros_like(rewards_array)
        for i in range (len(rewards_array)):
            discounted_reward_to_go[i] = np.sum(self.gamma**np.arange(len(rewards_array)-i) * rewards_array[i:])
