...
from amld2024.agents.pg_agent import PGAgent
...
from amld2024.infrastructure import utils
from amld2024.infrastructure.logger import Logger
from amld2024.infrastructure.action_noise_wrapper import ActionNoiseWrapper
...
def run_training_loop(args):
    ...
    for itr in range(args.n_iter):
        ...
        trajs, envsteps_this_batch = utils.sample_trajectories(env,agent.actor,args.batch_size,max_ep_len, render=True)
        total_envsteps += envsteps_this_batch
        total_envsteps_list.append(total_envsteps)
        ...
        if itr % args.scalar_log_freq == 0:
            ...
            average_returns_list.append(logs["Train_AverageReturn"])
            if args.use_baseline:
                baseline_loss_list.append(train_info["Baseline Loss"])
            print("Done logging...\n\n")
    ...
    return average_returns_list, total_envsteps_list, baseline_loss_list
...
def main():
    ...
    average_returns, total_envsteps_list, baseline_loss_list =  run_training_loop(args)
    
    # plot the average returns vs number of environment steps
    plt.figure()
    plt.plot(total_envsteps_list, average_returns)
    plt.xlabel('Number of environment steps')
    plt.ylabel('Average returns')
    exp_name = args.exp_name
    plt.title('Average returns vs number of environment steps: ' + exp_name)
    plt.show()
    
    # plot the baseline loss vs number of iterations
    if args.use_baseline:
        plt.figure()
        plt.plot(np.arange(args.n_iter), baseline_loss_list)
        plt.xlabel('Number of iterations')
        plt.ylabel('Baseline loss')
        plt.title('Baseline loss vs number of iterations: ' + exp_name)
        plt.show()
